...
export default function MultipleAnswers({ data }: { data: LessonState }) {
  ...
  const [randomized, setRandomized] = useState<{ italian: string; english: string; correct: boolean }[] | undefined>([])

  const removeItems = (word: string) => {
    const filtered = Object.entries(showSelected).filter(item => item[0] !== word && item)
  ...
  useEffect(() => {
    ...
    if (getType(activeExercise) && activeExercise?.targetNumber && Object.keys(showSelected).length > 0) {
  ...
  useEffect(() => {
    setRandomized(
      getType(activeExercise) ? activeExercise.availableWords.map(item => item).sort(() => Math.random() - 0.5) : undefined // or [] if you prefer an empty array over undefined
    )
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [activeExercise?._id])
          ...
          {randomized &&
            randomized.map(item => (
              <div
                ...
                onClick={activeExercise?.isComplete || activeExercise?.hasFailed ? undefined : e => handleSelectedItem(e, item.italian)}
              >
                <div
                  className={classNames(
                    (activeExercise?.isComplete || activeExercise?.hasFailed) && showSelected[item.italian] === item.italian
                      ? "border-color_purple_default border-2 text-color_purple_darker bg-purple-100 font-semibold cursor-not-allowed"
                      : (activeExercise?.isComplete || activeExercise?.hasFailed) && showSelected[item.italian] !== item.italian
                        ? "text-gray-800  cursor-not-allowed"
                        : activeExercise?.hasFailed && item.italian && item.correct
                ...
                >
                  ...
                  <p
                    className={classNames(
                      showSelected[item.italian] === item.italian ? "text-blue_border_darker border-blue_border" : "text-gray-400",
                      "text-sm  font-light"
                    )}
                  >
                    {activeExercise.type === "multipleAnswers" && activeExercise?.displayMeaning && item.english}
                  </p>
                  <p
                    className={classNames(
                      activeExercise?.isComplete && showSelected[item.italian] === item.italian
                        ? "text-green-500"
                        : activeExercise?.hasFailed && showSelected[item.italian] === item.italian && item.correct
                          ? "text-green-500"
                          : activeExercise?.hasFailed && item.italian && item.correct
                            ? "text-yellow-500"
                            : activeExercise?.hasFailed && showSelected[item.italian] === item.italian && !item.correct
                  ...
                  >
                    {activeExercise?.isComplete && showSelected[item.italian] === item.italian && item.correct
                      ? "CORRECT"
                      : activeExercise?.hasFailed && showSelected[item.italian] === item.italian && item.correct
                        ? "CORRECT"
                        : activeExercise?.hasFailed && item.italian && item.correct
