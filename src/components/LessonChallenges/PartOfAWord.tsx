...
export default function PartOfAWord({ data }: { data: LessonState }) {
  ...
  const matchMissing = (word: string) => getType(activeExercise) && activeExercise?.missing.some(item => item === word)
  ...
  useEffect(() => {
    ...
    if (getType(activeExercise) && opt.length === activeExercise?.missing[0].split("").length) {
