                  ...
                  <table className=" border border-t-1">
                    ...
                    <tbody className="bg-white">
                      {getType(activeExercise) &&
                        activeExercise.availableWords.pairs.map((item, index) => {
                          return (
                            <tr key={uuid()} className="">
                              <td className="px-3 py-1 sm:px-6  sm:py-3  font-medium  sm:text-lg text-gray-900">{results[index]}</td>
                              <td className="px-3 py-1 sm:px-6  sm:py-4  font-medium  sm:text-lg text-gray-900">{item[1]}</td>
      ...
      <InteractiveLayout id={activeExercise && activeExercise._id}>
        ...
        <div className="grid-cols-6 pb-6 px-4  items-center gap-1 text-sm  border-t-2 p-1  grid   sm:text-base sm:w-96">
          <h3 className="text-xl font-bold mt-3 w-full col-start-1 -ml-3 col-end-6 mb-3 bg-yellow-50 rounded-lg text-yellow-700 pl-4 py-2">
            {activeExercise?.type === "conjugation" && parse(activeExercise.display)}
          </h3>
              ...
              activeExercise.availableWords.pairs.map((item: any) => {
                return (
                  <div key={uuid()} className="mb-2">
                    <p className="text-gray-700 font-semibold px-1 border-2 border-white text-lg tracking-wider">{item[0]}</p>
                  </div>
                )
              })}
          </div>
          <div className="col-start-2">
            {activeExercise?.type === "conjugation" &&
              activeExercise.availableWords.pairs.map((item: any, index: number) => {
                return (
                  <div key={uuid()} className="mb-2">
                    <input
                      ...
                      className={classNames(
                        !index ? "bg-white border-2 border-black" : "border-2 border-black",
